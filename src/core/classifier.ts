 ...
 */
export function initialClassify(task: string): TaskSize {
  if (LARGE_PATTERNS.some((p) => p.test(task))) return 'large'
  if (SMALL_PATTERNS.some((p) => p.test(task))) return 'small'
  return 'medium'
}
 ...
 * ```
 */
export function extractPlannerRecommendation(
  planOutput: string
  ...
  try {
    const parsed = JSON.parse(raw) as Partial<PlannerRecommendation>
    if (
      (parsed.recommended_size === 'small' ||
