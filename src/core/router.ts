...
import { loadDelegateFleet, findLaneForRole } from '../discovery/delegate.js'
import { discoverAgents } from '../discovery/agents.js'

// ─── Effort tables ────────────────────────────────────────────────────────────

const EFFORT_TABLE: Record<BudgetMode, Record<Phase, EffortLevel>> = {
  conservative: {
    plan:      'medium',
    implement: 'medium',
    review:    'high',
...
const PHASE_MAP: Record<TaskSize, Phase[]> = {
...
// ─── Agent resolution (priority chain) ───────────────────────────────────────
...
function resolveAgent(opts: ResolutionOptions): { agent: string; execution: ExecutionMode } {
  const { phase, useDelegate, userConfig } = opts

  // 1. User explicit override
  const override = userConfig?.agentOverrides?.[phase as keyof NonNullable<UserConfig['agentOverrides']>]
  if (override) {
  ...
  const capable = agents
    ...
    .sort((a, b) => {
      ...
      const relevantScore = (s: typeof sa) =>
        phase === 'plan' || phase === 'review' || phase === 'verify'
          ? s.planning + s.review
          : s.coding
...
export function buildRoutingTable(opts: RouterOptions): PhaseRoute[] {
...
export function getAgentRole(currentModel: string) {
