...
function readFleet(path: string): FleetLane[] {
  if (!existsSync(path)) return []

  try {
    const raw = readFileSync(path, 'utf8')
    const fleet = yaml.load(raw) as RawFleet
    ...
    return Object.entries(lanes).map(([name, cfg]) => ({
      ...
      model:  cfg.model,
      effort: cfg.effort as FleetLane['effort'],
    }))
  } catch {
    return []
  }
 ...
 */
export function loadDelegateFleet(): FleetLane[] {
  ...
  const map = new Map<string, FleetLane>()
  for (const lane of global)  map.set(lane.name, lane)
  for (const lane of project) map.set(lane.name, lane)
 ...
 */
export function isDelegateAvailable(): boolean {
 ...
 */
export function findLane(name: string): FleetLane | null {
 ...
 */
export function findLaneForRole(role: string): FleetLane | null {
  const lanes = loadDelegateFleet()
  return (
    lanes.find((l) => l.name.toLowerCase().includes(role.toLowerCase())) ??
    lanes[0] ??
    null
  )
}
