...
export function detectProjectType(cwd: string = process.cwd()): ProjectType {
  if (has(cwd, 'pubspec.yaml'))   return 'flutter'
  if (has(cwd, 'package.json'))   return 'node'
  if (has(cwd, 'composer.json'))  return 'laravel'
  if (has(cwd, 'pyproject.toml') || has(cwd, 'setup.py') || has(cwd, 'requirements.txt'))
                                   return 'python'
  return 'unknown'
}

function has(dir: string, filename: string): boolean {
 ...
 */
export const VERIFY_COMMANDS: Record<ProjectType, string[]> = {
 ...
 */
export function getVerifyCommands(type: ProjectType): string[] {
