...
export function loadRegistry() {
    if (_registry)
        return _registry;
    const builtin = loadYaml(BUILTIN_PATH);
 ...
 */
export function getPhaseRequirement(phase) {
    const registry = loadRegistry();
 ...
 */
export function modelMeetsPhase(modelId, phase) {
    const score = getModelScore(modelId);
    const req = getPhaseRequirement(phase);
    if (req.min_planning && score.planning < req.min_planning)
        return false;
    if (req.min_coding && score.coding < req.min_coding)
        return false;
    if (req.min_review && score.review < req.min_review)
        return false;
    return true;
}
/**
 ...
 */
export function resolveAgentRole(currentModel) {
    ...
    if (canPlan && canReview && canCode)
        return 'full-orchestrator';
    if (canPlan || canReview)
        return 'partial-orchestrator';
