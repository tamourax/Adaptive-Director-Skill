...
import { readMetadata, buildBrief, writePhaseResult, updateMetadata, listRuns, } from '../core/handoff.js';
...
// ─── Resume ───────────────────────────────────────────────────────────────────
export async function resume(runIdOrLast) {
    // Resolve run ID
    let runId = runIdOrLast;
    if (runIdOrLast === 'last') {
        const runs = listRuns();
        const interrupted = runs.find((r) => r.status === 'interrupted' || r.status === 'running');
        if (!interrupted) {
    ...
    const phases = meta.routing.map((r) => r.phase);
